// Ported from gmx-subgraph/synthetics-stats/src/utils/time.ts
// i32 integer arithmetic preserved with Math.floor (JS `/` is floating point).
...
export function timestampToPeriodStart(timestamp: number, period: string): number {
  ...
  // in GMX weekly distributions start on Wednesdays
  ...
  let start = Math.floor(timestamp / seconds) * seconds;
