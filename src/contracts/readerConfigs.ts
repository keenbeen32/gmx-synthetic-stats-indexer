// Ported from gmx-subgraph/synthetics-stats/src/contracts/readerConfigs.ts
// Re-keyed by chain id (was keyed by network name). Addresses unchanged.

export class ReaderContractConfig {
  constructor(
...
export function getReaderContractConfigByChainId(chainId: number): ReaderContractConfig {
  const contract = readerContractByChainId[chainId];
  if (!contract) {
    throw new Error(`Reader contract config not found for chain ${chainId}`);
  }
  return contract;
}
