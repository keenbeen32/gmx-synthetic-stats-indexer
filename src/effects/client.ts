...
const rpcUrlByChainId: Record<number, string> = {
  42161: process.env.ENVIO_RPC_URL_42161 ?? "https://arb1.arbitrum.io/rpc",
...
export function effectRateLimit(): false | { calls: number; per: "second" } {
  const v = process.env.ENVIO_RPC_RATE_LIMIT;
  if (v && Number(v) > 0) {
...
const BATCH_SIZE = Number(process.env.ENVIO_RPC_BATCH_SIZE ?? 50);
